...
// Backend API URL
const API_URL = 'http://localhost:5000';  // Update this if your server runs on a different host/port

// Backend Integration: Fetch Classes
...
// Backend Integration: Add New Class
async function addClass() {
    ...
    try {
        await fetch(`${API_URL}/classes`, {
            ...
            body: JSON.stringify({ name: newClassName, students: [] })
        });

        alert('Class added successfully');
        closePopup();
        populateClasses();  // Refresh the class list
    } catch (error) {
        console.error('Error adding class:', error);
    }
}

// Backend Integration: Add Student to Class
async function addStudent() {
    ...
    try {
        await fetch(`${API_URL}/students/${selectedClass}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: newStudentName, rollNumber: newStudentRoll })
        });

        alert('Student added successfully');
        closePopup();
        showStudentsList();  // Refresh the student list
    } catch (error) {
        console.error('Error adding student:', error);
    }
}

// Backend Integration: Submit Attendance
async function submitAttendance() {
    const classSelector = document.getElementById('classSelector');
    const selectedClass = classSelector.options[classSelector.selectedIndex].value;
    const date = new Date().toISOString().split('T')[0];

    const attendanceRecords = Array.from(document.querySelectorAll('#studentsList li')).map(item => ({
        student: item.querySelector('strong').innerText,
        status: item.style.backgroundColor === 'rgb(46, 204, 113)' ? 'present'
            : item.style.backgroundColor === 'rgb(231, 76, 60)' ? 'absent'
            : 'leave'
    }));
    ...
    try {
        await fetch(`${API_URL}/attendance`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ className: selectedClass, date, records: attendanceRecords })
        });

        alert('Attendance submitted successfully');
        document.getElementById('studentsList').innerHTML = '';  // Clear student list
    } catch (error) {
        console.error('Error submitting attendance:', error);
    }
}

// Original function to show the list of students (with slight backend adjustments)
...
// Helper function to create attendance buttons (kept from original code)
function createButton(text, status, onClick) {
    const button = document.createElement('button');
    button.type = 'button';
    button.innerText = text;
